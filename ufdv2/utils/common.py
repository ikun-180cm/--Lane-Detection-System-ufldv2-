import os, argparse
# from ufdv2.data.dali_data import TrainCollect
from ufdv2.utils.dist_utils import get_rank, get_world_size, is_main_process, dist_print, DistSummaryWriter
...
def get_ufld_cfg(config_file=None):
    """
    CPU 推理专用，不触发 argparse
    """
    if config_file is None:
        # 设置默认配置文件
        config_file = "ufdv2/configs/curvelanes_res18.yaml"
    cfg = Config()
...
import importlib
def get_model(cfg):
    return importlib.import_module('ufdv2.model.model_'+cfg.dataset.lower()).get_model(cfg)
